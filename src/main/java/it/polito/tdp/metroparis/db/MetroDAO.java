...
public class MetroDAO {
	...
	public List<Connessione> getAllConnessioni(List<Fermata> fermate) {
		final String sql = "SELECT id_connessione, id_linea, id_stazP, id_stazA FROM connessione WHERE id_stazP>id_stazA"; 
		//con il ">" escludo le coppie inverse di stazioni
		...
		try {
			...
			while (rs.next()) {
				...
				Connessione c = new Connessione(rs.getInt("id_connessione"),
						null, //ignoro la linea, adesso non ci serve
						fermata_partenza,
						fermata_arrivo);
				result.add(c);
			}
